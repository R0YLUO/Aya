...
- [@aya/evals](./packages/evals.md) _(evals)_ — Eval workspace — versioned JSON datasets, deterministic scorers (CER, boundary F1 + idiom-split, reconstruction), an offline-capable runner over the real runOcr/analyzeText, and optional LangSmith dataset registration. `npm run eval` works locally with no keys.
