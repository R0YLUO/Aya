...
- [@aya/api](./packages/api.md) _(api)_ — Transport-agnostic handlers (health, uploads, pages/scan, shares-create, shares-resolve built; router todo), ApiError→envelope mapping, DynamoDB repository, S3 presign, short-URL service.
...
- [Share flow (end to end)](./concepts/share-flow.md) _(api, web, mobile)_ — POST /shares persists page+phrases+share transactionally and mints /s/{code}; web resolves it SSR via GET /shares/{code}. Both handlers built; only mobile share UI + router todo.
