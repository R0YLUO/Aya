...
tasks: [shared-reconstruction-invariant, llm-analysis-schema-and-prompt, llm-analyze-text, evals-reconstruction-and-report]
...
## Enforcement map (as of 2026-06-13)
...
| Evals | **Done** — `evals-reconstruction-and-report`: every analysis example carries `passed = reconstructionOk`; a reconstruction failure (`analyzeText` throws `AnalysisFailedError`) auto-fails the example regardless of quality scores, and the run-level baseline gate floors `analysisReconstructionPassRate` at 1.0 |
